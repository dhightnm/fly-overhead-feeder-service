...
import { AircraftState, DataSubmissionPayload, DataSubmissionResponse, FeederInfo } from './types';
...
export class FeederClient {
  ...
  constructor(config: FeederClientConfig) {
    ...
    this.api = axios.create({
      ...
      headers: {
        ...
        'User-Agent': '@fly-overhead/feeder-sdk/1.0.0',
      },
    });
  }
  ...
  /**
   * Get data quality feedback
   */
  async getQuality(): Promise<any> {
    const response = await this.api.get('/api/v1/feeders/me/quality');
    return response.data;
  }
